...
public final class LegaSqlHelper {
    ...
    public static Wrapper<?> fillWrapper(QueryCondition queryCondition, Wrapper<?> wrapper) {
        ...
        if (ObjectUtils.isNotEmpty(queryCondition.getAscs())) {
            qw.orderByAsc(queryCondition.getAscs());
        }
        if (ObjectUtils.isNotEmpty(queryCondition.getDescs())) {
            qw.orderByDesc(queryCondition.getDescs());
        }
        // QueryCondition 参数查询
     ...
     * @param conditions
     * @param wrapper
     * @return com.baomidou.mybatisplus.mapper.com.gcs.cloud.base.auth.server.mapper.Wrapper<?>
     * @description
     * @author: hsc
